...
// The vault file path is passed via $env:KES_VF (never string-interpolated into the
...
export async function setKey(service, account, secret) {
  ...
  if (PLATFORM === 'darwin') {
    // Note: the macOS `security` CLI takes the secret as an argv value (-w), so it is
    // briefly visible to `ps` on a shared machine during the call. Acceptable on a
    // single-user Mac; on shared hosts prefer an env var or a custom KES_LLM_API_KEY
    // flow. (The CLI has no clean stdin path for add-generic-password.)
    await sh('security', ['add-generic-password', '-U', '-s', NS + service, '-a', acct, '-w', String(secret)]);
    ...
    await ps(
      `Add-Type -AssemblyName System.Security; $s=[Console]::In.ReadToEnd(); $b=[Text.Encoding]::UTF8.GetBytes($s); $e=[Security.Cryptography.ProtectedData]::Protect($b,$null,'CurrentUser'); [IO.File]::WriteAllText($env:KES_VF,[Convert]::ToBase64String($e))`,
      String(secret),
      { KES_VF: f }
    );
...
export async function getKey(service, account) {
  ...
  try {
    ...
    if (PLATFORM === 'win32') {
      ...
      return (
        await ps(
          `Add-Type -AssemblyName System.Security; $e=[Convert]::FromBase64String([IO.File]::ReadAllText($env:KES_VF)); $b=[Security.Cryptography.ProtectedData]::Unprotect($e,$null,'CurrentUser'); [Console]::Out.Write([Text.Encoding]::UTF8.GetString($b))`,
          null,
          { KES_VF: f }
        )
      ).trim();
