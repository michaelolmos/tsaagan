...
// layers bootstrap this layer (Tsaagan can browse a site's developer settings,
...
export async function apiCall({ service, account, method = 'GET', url, path: p, body, headers = {} }) {
  ...
  if (!key) return { ok: false, error: `no key stored for "${service}" — run: tsaagan vault set service=${service} secret=…` };

  const full = url || (prov.base || '') + (p || '');
  if (!full) return { ok: false, error: 'need url= or a known service with a base + path=' };

  // Don't ship the stored secret to a host the provider isn't registered for. When the
  // provider has a `base`, the request host must share its registrable domain (so e.g.
  // api.github.com↔uploads.github.com is fine, api.openai.com→evil.com is not). Providers
  // with an empty base (e.g. supabase, ad-hoc services) require an absolute url= and have
  // no host to pin against, so they keep attaching as before.
  const regDomain = (host) => host.split('.').slice(-2).join('.');
  if (prov.base) {
    let fullHost, baseHost;
    try {
      fullHost = new URL(full).hostname;
      baseHost = new URL(prov.base).hostname;
    } catch {}
    if (!fullHost || !baseHost || regDomain(fullHost) !== regDomain(baseHost))
      return { ok: false, error: `refusing to send "${service}" key to ${fullHost || full} (not on ${baseHost || prov.base})` };
  }
  ...
  try {
    res = await fetch(full, {
      method,
      headers: h,
      // Don't auto-follow 3xx: fetch re-sends the credential header across the redirect,
      // which could forward the key to a different origin.
      redirect: 'manual',
