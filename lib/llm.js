// Unified LLM client for Kestrel's autonomous brain (planner / navigator /
// validator / reflection). Provider-agnostic over any OpenAI-compatible chat API.
//
// Pick a provider with KES_LLM_PROVIDER (or just supply one provider's key):
...
//   custom      KES_LLM_BASE_URL (+ KES_LLM_API_KEY)  Together / vLLM / Ollama / LM Studio / …
//
...
//
// Model per role resolves: KES_<ROLE>_MODEL → GROQ_<ROLE>_MODEL (back-compat) → the
...
const PROVIDERS = {
  ...
  openrouter: {
    ...
    headers: { 'HTTP-Referer': 'https://github.com/michaelolmos/kestrel', 'X-Title': 'Kestrel' },
    // Open-source default; for enterprise models pick a slug like anthropic/claude-3.7-sonnet,
    // openai/gpt-4o, google/gemini-2.0-flash via KES_*_MODEL.
...
export function info() {
  if (process.env.KES_LLM_BASE_URL) {
    return {
      provider: 'custom',
      url: process.env.KES_LLM_BASE_URL,
      key: process.env.KES_LLM_API_KEY || process.env.OPENROUTER_API_KEY || process.env.GROQ_API_KEY || '',
      headers: {},
      models: PROVIDERS.openrouter.models,
    };
  }
  let name = (process.env.KES_LLM_PROVIDER || '').toLowerCase();
  name = ALIAS[name] || name;
  if (name && PROVIDERS[name]) {
    const p = PROVIDERS[name];
    return { provider: name, url: p.url, key: firstKey(p.keys) || process.env.KES_LLM_API_KEY || '', headers: p.headers, models: p.models };
  }
  ...
  // Google / Anthropic / xAI, name it via KES_LLM_PROVIDER.
...
export function modelFor(role) {
  ...
  const r = info();
  return process.env[`KES_${R}_MODEL`] || process.env[`GROQ_${R}_MODEL`] || (r.models || PROVIDERS.openrouter.models)[role];
}
...
export async function chat(model, messages, { json = true, temperature = 0.2 } = {}) {
  ...
  if (!p.key && p.provider !== 'custom') {
    throw new Error('no LLM API key — set GROQ_API_KEY / OPENROUTER_API_KEY / OPENAI_API_KEY / GEMINI_API_KEY / ANTHROPIC_API_KEY / XAI_API_KEY (and KES_LLM_PROVIDER for the enterprise ones), or KES_LLM_BASE_URL for a custom endpoint.');
  }
