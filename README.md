<p align="center">
  <img src="docs/hero.png" alt="Tsaagan — human-like browser control for AI agents" width="100%">
</p>
...
> **Tsaagan gives your AI agent real hands on the web** — so it can finish the
...
**Human-like browser control for AI agents.** Tsaagan lets an AI drive a real
...
> **The mental model: your agent is the brain, Tsaagan is the hands.** Whatever AI
> drives it (Claude, a Codex/ChatGPT CLI, Cursor, or any model) supplies intent and
> judgment; Tsaagan supplies reliable **perception → action → verification → memory**.
> The agent decides *what*; Tsaagan reliably does *how*, and proves it worked.

![Tsaagan performing a verified multi-step task](docs/demo.gif)
...
### The mental model — Tsaagan is the hands; your agent is the brain
...
submit that"), the agent stalls and *you* become the hands. Tsaagan closes that gap.
...
### What Tsaagan deliberately is **not**

Tsaagan is **not** a tool for defeating security, solving CAPTCHAs, evading bans, or
...
## Philosophy — why I built Tsaagan
...
**Tsaagan exists to give your AI agents real hands** — so they can autonomously do
...
And it's built on a clear ethic: **empowerment, not abuse.** Tsaagan deliberately does
...
> **Use it on tasks you're authorized to do.** Tsaagan is for automating *your own*
...
![Tsaagan capability layers](docs/layers.svg)

1. **API layer** — if a site has an official API, Tsaagan calls it directly with a
...
Plus an **agent layer** (identity + brain memory) that makes Tsaagan a self-improving
agent, not just a tool. → [docs/AGENT.md](docs/AGENT.md)

## Memory that learns (a real loop, not just storage)

![Tsaagan's learning loop](docs/learning-loop.svg)

Tsaagan doesn't just remember — it **learns from each run and feeds the lesson back**:
...
- **Research-grade recall:** ranked by **relevance (embeddings) × importance × recency**,
  ...
  `TSG_EMBED_URL` or `@xenova/transformers` is present.
...
Memory lives on your machine (`~/.tsaagan/brain.db`) — not in a model, not in the cloud.
...
the model's guess* that an action succeeded. Tsaagan fixes both:

- **Stable, self-healing grounding.** Elements are addressed by accessibility-tree
  refs; if a ref goes stale, Tsaagan re-snapshots and re-locates the same element
  by role + name automatically.
- **Structural verification.** After every action Tsaagan returns a `verify`
...
## How Tsaagan compares

The browser-agent space is crowded — and Tsaagan is the new, small project here. It
...
other tool lacks — structural proof that each action worked — is Tsaagan's core.

| Capability | Tsaagan | browser-use | stagehand | playwright-mcp | skyvern |
|---|:---:|:---:|:---:|:---:|:---:|
...
> **Honest notes.** Tsaagan is young — fewer stars and no hosted
...
![Tsaagan architecture](docs/architecture.svg)

Tsaagan is **one engine you can drive several ways**:

- **The tool** — a CLI (`tsaagan.js`) talking to a background **daemon** that holds a
...
- **As a skill** — a short instruction file that teaches your AI *how* to drive the
  tool (the observe → act → verify loop). "Use the Tsaagan skill" → the AI reads it
  and drives Tsaagan step by step. (A ready-made skill for Claude Code is included.)
...
- **As an MCP server or SDK** — `tsaagan mcp` exposes the verify-first verbs to Claude
...
| `tsaagan run` / `tsaagan serve` (autonomous) | **any** OpenAI-compatible LLM — Groq · OpenRouter · OpenAI · Google Gemini · Anthropic · xAI Grok · local | a key for your chosen provider |

Tsaagan itself makes **no LLM calls** when an agent drives it — it's pure
...
`TSG_LLM_BASE_URL`. Pick one with `TSG_LLM_PROVIDER=groq|openrouter|openai|google|anthropic|xai`.
See [docs/AGENT.md](docs/AGENT.md#brain-provider).

## Use it with any coding agent

Tsaagan is just a terminal program, so any agent that can run shell commands can
drive it. Tell your agent something like:

> "Use Tsaagan (`node tsaagan.js`) to log into example.com and download my invoices.
> Snapshot, act on refs, and check the `verify` block after each step."

It will run `tsaagan start`, `tsaagan snapshot`, `tsaagan click ref=…`, read the
...
```bash
git clone https://github.com/michaelolmos/tsaagan.git && cd tsaagan
npm install
...
```bash
node tsaagan.js start                    # launch browser (fresh isolated profile)
node tsaagan.js goto url=https://example.com expectText="Example"
node tsaagan.js snapshot                 # accessibility tree with [ref=eN]
node tsaagan.js click ref=e6 expectText="IANA"
node tsaagan.js type ref=e3 text="hello" submit=true
node tsaagan.js stop
```

Add an alias: `alias tsaagan="node $PWD/tsaagan.js"`.

## Use it from Claude Desktop, Claude Code, or Cursor (MCP)

Tsaagan ships a built-in **MCP server**, so any [Model Context Protocol](https://modelcontextprotocol.io)
host can drive the browser directly. Unlike other browser MCP servers, **every mutating
tool returns Tsaagan's `verify` block** (URL changed? console errors? expected text?) —
proof the action worked, in the same response, with no extra snapshot round-trip.

**Claude Code:**

```bash
claude mcp add tsaagan -- node /path/to/tsaagan/tsaagan.js mcp
# or, if you've installed the `tsaagan` bin globally:  claude mcp add tsaagan -- tsaagan mcp
```
...
{
  "mcpServers": {
    "tsaagan": { "command": "node", "args": ["/path/to/tsaagan/tsaagan.js", "mcp"] }
  }
}
```

The server **auto-starts a headless daemon** on first use. It exposes 21 verify-first
tools — `tsaagan_navigate`, `tsaagan_snapshot`, `tsaagan_click`, `tsaagan_fill_form`,
`tsaagan_extract`, `tsaagan_network` (discover a site's own data API), and more.
Set `TSAAGAN_HEADLESS=0` to watch it work, or `TSG_TOKEN` to lock the control plane on
shared machines. See [docs/PROTOCOL.md](docs/PROTOCOL.md).

## Use it from your own code (JS & Python SDK)

Prefer calling Tsaagan programmatically? Both clients return a `verify` block on
...
```js
import { createTsaagan } from 'tsaagan/sdk';   // or '/path/to/tsaagan/sdk/index.mjs'

const k = await createTsaagan();                // auto-starts a headless daemon
...
**Python** (`pip install tsaagan`, zero dependencies):
...
The Python client needs the Node daemon reachable — set `TSAAGAN_JS=/path/to/tsaagan.js`
or put the `tsaagan` binary on PATH. See [clients/python/](clients/python/).
...
export OPENROUTER_API_KEY=...   TSG_LLM_PROVIDER=openrouter   # open-source models
export OPENAI_API_KEY=...       TSG_LLM_PROVIDER=openai       # GPT-4o / o-series
export GEMINI_API_KEY=...       TSG_LLM_PROVIDER=google       # Gemini
export ANTHROPIC_API_KEY=...    TSG_LLM_PROVIDER=anthropic    # Claude
export XAI_API_KEY=...          TSG_LLM_PROVIDER=xai          # Grok
export TSG_LLM_BASE_URL=http://localhost:11434/v1/chat/completions  # Ollama, fully local

node tsaagan.js run goal="go to news.ycombinator.com and report the #1 story title"
node tsaagan.js journal                   # what it has done before (persistent memory)
```

Per-role model overrides work with any provider: `TSG_PLANNER_MODEL`,
`TSG_NAV_MODEL`, `TSG_VALIDATOR_MODEL` (e.g. point the planner at a stronger model
for reasoning while a fast model handles navigation).

## Standalone server (no agent framework)

```bash
node tsaagan.js serve port=39820
...
```bash
node tsaagan.js bench          # autonomous task suite + deterministic capability matrix
```
...
| `totp secret=$TSG_TOTP_SECRET` · `detect_captcha` | 2FA code · detect a CAPTCHA/anti-abuse wall → hand off to a human |
...
| `advise [domain=..] [query=..]` | what Tsaagan learned about a site (also auto-surfaced in every `snapshot.memory`) |
...
tsaagan record_stop                         # writes ~/.tsaagan/records/*.json
tsaagan replay path=/Users/me/.tsaagan/records/...
tsaagan report format=md                    # writes ~/.tsaagan/reports/*.md
```
...
```bash
tsaagan doctor                              # local install/config health check
tsaagan protocol                            # paths to protocol/actions.schema.json + protocol/tsaagan.d.ts
```
...
`ANTHROPIC_API_KEY` · `XAI_API_KEY`/`GROK_API_KEY` — or `TSG_LLM_BASE_URL`
(+ `TSG_LLM_API_KEY`) for any OpenAI-compatible endpoint · `TSG_LLM_PROVIDER=groq|openrouter|openai|google|anthropic|xai`
to name one · model overrides `TSG_PLANNER_MODEL` / `TSG_NAV_MODEL` / `TSG_VALIDATOR_MODEL`
(legacy `GROQ_*_MODEL` still honored).
**Other:** `TSG_USER` / `TSG_PASS` / `TSG_TOTP_SECRET` (login) · `TSG_PORT` (daemon port) ·
`TSG_EMBED_URL` (optional embedding endpoint for semantic recall). State lives in `~/.tsaagan/`.
...
```
tsaagan.js  ── thin CLI ──▶  daemon.js  (one persistent Playwright page; HTTP control plane)
...
Tsaagan has three ways to drive the browser. The reason for more than one is purely
...
**Native mode (macOS)** — `tsaagan start mode=native`. Drives your **real Chrome with
...
**Extension mode (any OS)** — `tsaagan ext-setup`. A companion extension delivers
...
tsaagan start mode=native                   # drives your real, frontmost Chrome
tsaagan snapshot                            # AppleScript perception (elements + screen coords)
tsaagan click i=5                           # real OS-level trusted click
tsaagan paste text="..." submit=true        # clipboard + real ⌘V + Return
```

## Self-learning & memory

Tsaagan keeps a small, local memory under `~/.tsaagan/`:

- **Per-site memory** (`memory/<domain>.json`) — working selectors and notes.
  Tsaagan **auto-captures** a selector when an action succeeds (no manual step),
...
- **Auto-learned lessons** — if a site shows an anti-abuse / CAPTCHA / "unusual
  activity" wall, Tsaagan **automatically** writes a note to that domain and flips
...
- **Task journal** (`agent/journal.jsonl`) — every autonomous run is recorded;
  `tsaagan journal` shows recent history. `agent/learnings.json` holds distilled notes.
...
Tsaagan is a **productivity tool** — it lets your AI agent act like *you* in a
...
of Service. When Tsaagan meets a CAPTCHA or an anti-abuse wall, it **stops and hands
control back to you**. Keep your use authorized, attributed, and within each site's
terms.

> **Disclaimer.** Tsaagan is provided "as is", without warranty (see [LICENSE](LICENSE)).
...
Tsaagan drives a **real browser with real permissions**. Be deliberate:
...
- **Terms of Service & anti-abuse.** Automating sites can violate their ToS and trip
  ...
  responsibly, on sites/accounts you're permitted to automate. Tsaagan detects abuse
...
| [docs/BUILD_JOURNAL.md](docs/BUILD_JOURNAL.md) | How Tsaagan was built — the research, decisions, and every feature wave |
