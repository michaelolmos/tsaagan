// Thin bridge from the MCP server to the Kestrel daemon's localhost control
...
const AUTOSTART = process.env.KESTREL_MCP_AUTOSTART !== '0';
const MODE = process.env.KESTREL_MODE || 'fresh';
const HEADLESS = process.env.KESTREL_HEADLESS !== '0'; // headless by default for MCP use
const CALL_TIMEOUT_MS = parseInt(process.env.KESTREL_MCP_TIMEOUT || '60000', 10);
 ...
 * KESTREL_MCP_AUTOSTART=0 — spawn a detached headless daemon and wait for it to
...
export function ensureDaemon() {
  ...
  ensuring = (async () => {
    ...
    if (!AUTOSTART) return { ok: false, error: `no Kestrel daemon on ${BASE} and autostart is disabled (KESTREL_MCP_AUTOSTART=0). Run: kestrel start` };
    ...
    return { ok: false, error: 'daemon did not become ready in time; see ~/.kestrel/daemon.log' };
  })();
