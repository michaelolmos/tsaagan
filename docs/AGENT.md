# Agent layer — Tsaagan as a thinking agent

Tsaagan's core is a **tool** (the brain that drives it lives elsewhere). The agent
...
- **[SOUL.md](../SOUL.md)** — what Tsaagan is and why it exists (the intent), plus its
...
Tsaagan keeps **two complementary stores**, both local and inspectable:

| Store | Where | What | Format |
|---|---|---|---|
| **Site memory** | `~/.tsaagan/memory/<domain>.json` | learned selectors, notes, per-domain `pace` | JSON (human-editable) |
| **The brain** | `~/.tsaagan/brain.db` | episodic task history + learnings (for recall/evolution) | **SQLite** (built-in `node:sqlite`; falls back to `brain.jsonl`) |

So: memory is **contained in Tsaagan on your machine** — not inside a model, not in
...
```bash
tsaagan brain                       # stats (backend, episodes, learnings)
tsaagan brain recall query="login"  # past runs + learnings matching a query
```
...
2. **Reflect** — at the end of a run Tsaagan turns the outcome into reusable knowledge:
...
3. **Feed back** — and this is the part that makes it *learning*: the lessons are
   ...
   - explicit pull: `tsaagan advise` (daemon) / `tsaagan brain advise domain=…`.

So the lessons don't just sit in a database — they **change the next attempt**, and
they flow back to whatever brain is driving Tsaagan.
...
- **Relevance ranking, not substring match.** Recall ranks memories by
  **relevance (embedding cosine) × importance × recency** (Generative Agents-style),
  ...
  vectors if `TSG_EMBED_URL` (any embedding endpoint, e.g. a local sentence-transformer) or `@xenova/transformers`
  is available. Importance is weighted (anti-abuse/CAPTCHA > one-off notes).
- **Reflection-synthesis.** When lessons accumulate, Tsaagan compresses them into a
  few durable **domain rules** (`tsaagan brain synthesize domain=…`, also automatic
...
| Provider | `TSG_LLM_PROVIDER` | Key | Default model |
|---|---|---|---|
...
| **Custom** (Together / vLLM / Ollama / LM Studio) | — | `TSG_LLM_BASE_URL` (+ `TSG_LLM_API_KEY`) | set `TSG_*_MODEL` |

- **Auto-select** (no `TSG_LLM_PROVIDER`) is limited to Groq/OpenRouter so a stray
  enterprise key in your environment never silently changes which brain runs — to use
  OpenAI/Google/Anthropic/xAI, **name it** with `TSG_LLM_PROVIDER`. When both Groq and
  OpenRouter keys exist, Groq wins (back-compat).
- Per-role overrides (any provider): `TSG_PLANNER_MODEL`, `TSG_NAV_MODEL`,
  `TSG_VALIDATOR_MODEL`. Legacy `GROQ_*_MODEL` names still honored. E.g. via OpenRouter
...
You can use Tsaagan purely as a tool (drive the verbs yourself) and ignore this layer
